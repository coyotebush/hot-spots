...
for i, loc in enumerate(locs):
    ...
    print "\nProcessing point {} (preceded by {})".format(loc[0], prev[0])
    ...
    print "Inserted row {}".format(newid)
                   ...
                   # ORDER BY weight / GreatCircleLength(ExteriorRing(geom))
                   # LIMIT 1
    # Try merging this favorite into existing ones
    params = {"newid": newid, "locid": loc[0]}
    cur.execute("""UPDATE favorite
                   SET weight = weight + (SELECT weight FROM favorite WHERE id = :newid),
                     geom = ConvexHull(GUnion(geom, (SELECT geom FROM favorite WHERE id = :newid))),
                     last_location = :locid
                   WHERE id <> :newid
                   AND (weight / GreatCircleLength(ExteriorRing(geom))) <
                     ((weight + (SELECT weight FROM favorite WHERE id = :newid)) /
                     GreatCircleLength(ExteriorRing(ConvexHull(GUnion(geom,
                     (SELECT geom FROM favorite WHERE id = :newid))))))""",
                params)
                   # (SELECT (f.weight + g.weight) /
                   #    GreatCircleLength(ExteriorRing(ConvexHull(GUnion(f.geom, g.geom))))
                   #  FROM favorite g WHERE id = :newid)""")
    print "Updated {} rows".format(cur.rowcount)

    # If any of these merges was better, delete the new favorite
    cur.execute("""DELETE FROM favorite
                   WHERE id = :newid
                   AND (weight / GreatCircleLength(ExteriorRing(geom))) <
                     (SELECT MAX(weight / GreatCircleLength(ExteriorRing(geom)))
                      FROM favorite
                      WHERE last_location = :locid
                      AND id <> :newid)""", params)
    print "Deleted {} rows".format(cur.rowcount)
